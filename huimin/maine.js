...
function GameTimer()
{
    ...
    if (GameStatus)
    {
        setTimeout("GameTimer()", DifficultyLag);
        skillcd = skillcd - DifficultyLag;
        if (skillcd < 0) skillcd = 0;
        document.getElementById("Skill-cd").innerText = skillcd;
    }
}
...
function Tetris()
{
    ...
    this.EngiSkill = function ()
    {
        skillcd = 45000;
        for (var i=0;i<nHNumber;i++)
        {
            this.aState[i][nVNumber-1] = 1;
        }
        var nLine = 0;
        for (var i = nVNumber - 1; i >= 0; i--)
        {
...
function Action(event) 
{
    ...
    rushBlock.NullifyRect();
        switch (event.keyCode) 
        {
            ...
            case 67:
            {
                if ((GameStatus == 1) && (skillcd == 0)) rushBlock.EngiSkill();
                break;
            }
        }
    GameCanvasContext.fillStyle = "red";
